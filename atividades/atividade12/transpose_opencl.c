...
int main (int argc, char *argv[]) {
    ...
    if (argc != 5) {
		...
		printf("<N> :	Tamanho da matriz que será gerada NXN.\n");
		printf("<DEBUG> :	1 para mostrar a transposta;0 para não mostrar.\n");
		printf("%s <PLATAFORMA> <DISPOSITIVO> <N> <DEBUG>\n", argv[0]);
		exit(1);
	}
	...
	if (WINDOWS) fopen_s(&programHandle, "transpose_opencl.cl", "r");             // Alterei para fopen_s
	if (LINUX) programHandle = fopen("transpose_opencl.cl", "r");
